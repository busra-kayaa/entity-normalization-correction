...
def test_t5_small_categorized():
    ...
    model_path = (t5_small_folder / "final_model").as_posix()

    # Çıktı Dosya Yolları
    output_excel = reports_dir / "t5_small_manual_test_results.xlsx"
    output_text = reports_dir / "t5_small_manual_test_report.txt"

    if not os.path.exists(model_path):
        print(f"❌ Hata: Model yolu bulunamadı! {model_path}")
        return

    print("🔄 T5-Small Modeli ve Tokenizer yükleniyor...")
    ...
    print(f"🧪 {len(test_cases)} manuel senaryo T5-Small ile test ediliyor...\n" + "=" * 80)

    for input_raw, target, category in test_cases:
        if category not in category_stats:
            category_stats[category] = {"total": 0, "correct": 0}

        input_text = "gec: " + input_raw.strip()
    ...
    report_header = f"""
==================================================
      T5-SMALL MANUEL TEST KATEGORİ RAPORU
==================================================
Tarih: {report_time}
