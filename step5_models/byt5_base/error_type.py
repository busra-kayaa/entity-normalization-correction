...
def detailed_error_analysis_byt5_colab():
    # 1. Colab Klasör ve Dosya Yolları
    # Eğitim bittikten sonra modelin kaydedildiği klasör adı (Kendi klasör ismine göre ayarla):
    model_path = "/content/final_byt5_model"
    data_path = "/content/test.json"

    reports_dir = Path("/content/reports")
    reports_dir.mkdir(parents=True, exist_ok=True)
    report_output = reports_dir / "byt5_error_type_performance_detailed.xlsx"

    # 2. Cihaz ve Model Kurulumu
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"🚀 ByT5 Detaylı Analiz Başlatıldı (Cihaz: {device.upper()})")
    ...
    print(f"🔍 {len(test_data)} örnek ByT5 ile kategori bazlı analiz ediliyor...")
    for item in tqdm(test_data):
        ...
        # 🛡️ ByT5 YUTMA KORUMASI: Girdinin en başına bilerek boşluk ekliyoruz!
    ...
    print("\n" + "=" * 65)
    print(f"📈 ByT5 MODEL PERFORMANS ÖZETİ (Toplam Test: {len(test_data)})")
    print("=" * 65)
...
if __name__ == "__main__":
    detailed_error_analysis_byt5_colab()